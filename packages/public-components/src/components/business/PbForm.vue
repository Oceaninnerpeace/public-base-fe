  ...
  <a-form :model="formModel" :layout="layout" :label-col="{ style: { width: `${labelWidth}px` } }">
      ...
      <a-col v-for="schema in visibleSchemas" :key="schema.field" :span="schema.span ?? 24">
        ...
        >
          ...
          <PbDictSelect
            ...
            :model-value="formModel[schema.field] as string | number | undefined"
            @update:model-value="(v) => (formModel[schema.field] = v)"
          ...
          <a-date-picker
            ...
            v-bind="schema.componentProps"
          />
          <slot v-else :name="schema.field" :model="formModel" :schema="schema" />
        </a-form-item>
      </a-col>
    </a-row>
    <a-form-item v-if="showAction">
      <a-space>
        ...
        <slot name="action" :model="formModel" />
      </a-space>
    </a-form-item>
  </a-form>
</template>
